...
const BikeCategorise = ({bikes}) => {
  const [category,setCategory]=useState('all')
  const [filteredBikes,setFilteresBikes]=useState([])
          ...
          <aside className="  w-full p-4 mb-8 xl:w-[300px] xl:h-[84vh] xl:fixed">
           ...
           <div className="max-w-56">
            <div className=" text-lg mb-4 font-medium">
              ...
              <span className="ml-2">
                ({filteredBikes.length > 1 ? `${filteredBikes.length} items` : filteredBikes === 0 ? `${filteredBikes.length} items` : `${filteredBikes.length} item`})
              </span>
            </div>
